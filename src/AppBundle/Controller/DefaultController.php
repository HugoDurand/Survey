...
class DefaultController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if (isset($data['submit'])){
            $participant = $em->getRepository('AppBundle:Participants')->findOneByNumParticipant($data['numparticipant']);

            if($participant != NULL){
                $session = $em->getRepository('AppBundle:Session')->findOneById($participant->getSession());
            }

            $reponse =  $em->getRepository('AppBundle:Reponse')->findByNumParticipant($data['numparticipant']);
